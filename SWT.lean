...
lemma SubAlgClosureUnderAbs (F : Subalgebra ℝ C(Ω, ℝ)) (f : F.topologicalClosure) :
    |(f : C(Ω, ℝ))| ∈ F.topologicalClosure := by
  sorry


/- Proof of Max identity with absolute value identifies -/
lemma MaxIdentityAbs (f g : C(Ω, ℝ)) :
    2 * (f ⊔ g) = f + g + |f - g| := by
  sorry


/- Proof of Min identity with absolute value identifies -/
lemma MinIdentityAbs (f g : C(Ω, ℝ)) :
    2 * (f ⊓ g) = f + g - |f - g| := by
  sorry


/- Proof that a Subalgebra is topologically closed under pointwise maximum -/
lemma SubAlgClosureUnderMax (F : Subalgebra ℝ C(Ω, ℝ)) (f g : F.topologicalClosure) :
    ↑f ⊔ ↑g ∈ F.topologicalClosure := by
  sorry


/- Proof that a Subalgebra is topologically closed under pointwise minimum -/
lemma SubAlgClosureUnderMin (F : Subalgebra ℝ C(Ω, ℝ)) (f g : F.topologicalClosure) :
    ↑f ⊓ ↑g ∈ F.topologicalClosure := by
  sorry
...
lemma SubAlgClosureSeparatesPoints (F : Subalgebra ℝ C(Ω, ℝ)) (hF: F.SeparatesPoints) :
    F.topologicalClosure.SeparatesPoints := by
  sorry


/- The Stone-Weierstrass Theorem -/
theorem Stone_Weierstrass (F : Subalgebra ℝ C(Ω, ℝ)) (hF : F.SeparatesPoints) :
    F.topologicalClosure = C(Ω, ℝ) := by
  -- apply hF
  sorry
